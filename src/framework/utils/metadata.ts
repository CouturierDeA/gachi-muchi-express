...
import {AutowireOptions, ComponentDecoratorOptions} from "../component";
import {ApplyMappingOptions, ArgumentInjectionsType, ArgumentsInjector} from "../controller/controller";
import {InstanceCreator} from "./instance-creator";

const COMPONENT_DECORATOR = 'ComponentDecorator'

export function defineComponentDecorator<T extends { new(...args: any[]): {} }>(
    constructor: T,
    options?: ComponentDecoratorOptions
) {
    Reflect.defineMetadata(COMPONENT_DECORATOR, {
        options,
        constructor
    }, constructor)
}
...
export function getArgumentInjections<S>(
    target: any, propertyKey: string
) {
    return Reflect.getMetadata(ARGUMENT_INJECTIONS, target, propertyKey) as ArgumentInjectionsType<S> || {}
}
...
export function defineComponentInstanceCreator<T>(
    ...
    Reflect.defineMetadata(COMPONENT_INSTANCE_CREATOR, instanceCreator, constructor)
}

export function getComponentInstanceCreator<T>(constructor: T): InstanceCreator<T> {
...
export function defineControllerInstanceCreator<T extends { new(...args: any[]): {} }>(
    constructor: T,
    instanceCreator: InstanceCreator<T>
) {
...
export function getControllerInstanceCreator<T>(constructor: T): InstanceCreator<T> {
...
export function defineRouteMeta<T extends { new(...args: any[]): {} }>(
    ...
    const newMeta: RouteMetaOptions<T>[] = [
        ...routeMeta,
        options
    ]
...
const AUTOWIRE_DECORATOR = 'AUTOWIRE'

export function getAutoWire<T>(constructor: T): AutowireOptions[] {
    return Reflect.getMetadata(AUTOWIRE_DECORATOR, constructor) || []
}

export function defineAutoWire<T>(
    constructor: T,
    options: AutowireOptions
) {
    Reflect.defineMetadata(AUTOWIRE_DECORATOR, [
        ...getAutoWire(constructor),
        options
    ], constructor)
}
...
export function defineMappingMeta<T>(
    ...
    Reflect.defineMetadata(MAPPING_DECORATOR, [
        ...mappingMeta,
        {
            propertyKey,
            options
